...
describe "Header" do
  ...
  it "should have a published product type" do
    ...
    product = Factory.build(:product, :header_type => nil)
    product.should_not be_valid

    # Should not be valid with an unpublished product type
    header_type = Factory.build(:header_type, :published => false)
    product.header_type = header_type
    product.should_not be_valid
  ...
  it "should search by item number (not name) in content library" do
    ...
    Header.search(:term => 'foo').find_all{|w| w == product }.size.should be(1)

    # When I search for the name the product should not be in the result set
    Header.search(:term => 'bar').find_all{|w| w == product }.size.should be(0)
  end
end
